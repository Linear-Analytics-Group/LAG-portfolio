...
field schema. Everything else — CSV reading, dedup, the upsert loop, and
settings/auth/logging orchestration — is inherited unchanged from
``BaseInventorySyncRunner`` and
``lag_service_kit.runners.base.BaseSyncRunner``.
"""
...
class DataverseInventorySyncRunner(BaseInventorySyncRunner):
    """Syncs ERP inventory records into Microsoft Dataverse.

    Notes
    -----
    A future destination (e.g. SAP, Salesforce) is added by writing a
    sibling module — ``runners/sap.py``, ``runners/salesforce.py`` — that
    subclasses ``BaseInventorySyncRunner`` the same way this class does,
    supplying that destination's own settings, client, entity set,
    alternate key, and field mapping.
    """
