...
def test_csv_record_reader_loads_expected_columns_and_values(tmp_path):
...
def test_csv_record_reader_raises_file_not_found_for_missing_path(tmp_path):
...
def test_json_record_reader_loads_orient_records_layout(tmp_path):
...
def test_json_record_reader_raises_file_not_found_for_missing_path(tmp_path):
    """Reading a nonexistent JSON path raises FileNotFoundError."""
    with pytest.raises(FileNotFoundError):
        JsonRecordReader().load(tmp_path / "does-not-exist.json")


def test_parquet_record_reader_loads_expected_columns_and_values(tmp_path):
...
def test_parquet_record_reader_raises_file_not_found_for_missing_path(tmp_path):
...
def test_every_reader_satisfies_the_record_reader_protocol(reader_cls):
